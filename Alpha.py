...
# ============================================================
# EVENT CYCLE + TD SEQUENTIAL ENGINE
# ============================================================
def _normalize_ohlcv(price_df):
    """Return a clean daily OHLCV frame without changing the caller's data."""
    if price_df is None or price_df.empty:
    ...
    required = [c for c in ["Date", "Open", "High", "Low", "Close"] if c in df.columns]
...
def calculate_td_sequential(price_df, setup_target=9, countdown_target=13):
    """Calculate a transparent daily TD Sequential-style 9/13 series.

    Setup: 9 consecutive closes below/above the close four bars earlier.
    Countdown: non-consecutive closes <= low / >= high two bars earlier.

    This intentionally exposes the raw counts and does not hide the signal behind
    a proprietary/opaque score. It is a practical research implementation; full
    DeMARK Sequential has additional qualifiers, cancellation, recycling and risk
    rules that can be layered on later.
    """
    df = _normalize_ohlcv(price_df)
    if df.empty:
        return df
    ...
    setup_perfected = [False] * n
    countdown_buy = [0] * n
    countdown_sell = [0] * n
    td_countdown = [0] * n
    countdown_dir = [None] * n
    countdown_complete = [False] * n
    ...
    for i in range(n):
        # Setup is strictly consecutive. A failed condition resets the direction.
        if i >= 4 and closes[i] < closes[i - 4]:
            setup_buy[i] = setup_buy[i - 1] + 1 if i > 0 and setup_dir[i - 1] == "Buy" else 1
            setup_sell[i] = 0
            setup_dir[i] = "Buy"
        elif i >= 4 and closes[i] > closes[i - 4]:
            setup_sell[i] = setup_sell[i - 1] + 1 if i > 0 and setup_dir[i - 1] == "Sell" else 1
            setup_buy[i] = 0
            setup_dir[i] = "Sell"
        else:
            setup_buy[i] = 0
        ...
        td_setup[i] = setup_buy[i] if setup_buy[i] else setup_sell[i]
        if setup_buy[i] == setup_target:
            setup_complete[i] = True
            if i >= 8:
                setup_perfected[i] = (lows[i - 1] < lows[i - 3] and lows[i - 1] < lows[i - 2]) or \
                                     (lows[i] < lows[i - 2] and lows[i] < lows[i - 3])
            active_countdown = "Buy"
            buy_cd = 1 if i >= 2 and closes[i] <= lows[i - 2] else 0
            sell_cd = 0
        elif setup_sell[i] == setup_target:
            setup_complete[i] = True
            if i >= 8:
                setup_perfected[i] = (highs[i - 1] > highs[i - 3] and highs[i - 1] > highs[i - 2]) or \
                                     (highs[i] > highs[i - 2] and highs[i] > highs[i - 3])
            active_countdown = "Sell"
            sell_cd = 1 if i >= 2 and closes[i] >= highs[i - 2] else 0
            buy_cd = 0
        else:
            # Opposite completed setup cancels an in-progress countdown.
            if setup_buy[i] == 1 and active_countdown == "Sell":
                active_countdown = None
                sell_cd = 0
            elif setup_sell[i] == 1 and active_countdown == "Buy":
                active_countdown = None
                buy_cd = 0

        # Continue an active countdown. Countdown is non-consecutive.
        if active_countdown == "Buy" and not setup_complete[i]:
            if i >= 2 and closes[i] <= lows[i - 2] and buy_cd < countdown_target:
                buy_cd += 1
        elif active_countdown == "Sell" and not setup_complete[i]:
            if i >= 2 and closes[i] >= highs[i - 2] and sell_cd < countdown_target:
                sell_cd += 1

        countdown_buy[i] = min(buy_cd, countdown_target)
        countdown_sell[i] = min(sell_cd, countdown_target)
        if buy_cd:
            td_countdown[i] = buy_cd
            countdown_dir[i] = "Buy"
        elif sell_cd:
            td_countdown[i] = sell_cd
            countdown_dir[i] = "Sell"

        if buy_cd >= countdown_target:
            countdown_complete[i] = True
            active_countdown = None
        elif sell_cd >= countdown_target:
            countdown_complete[i] = True
            active_countdown = None

    df["TD Setup"] = td_setup
    df["TD Setup Direction"] = setup_dir
    df["TD Setup Complete"] = setup_complete
    df["TD Setup Perfected"] = setup_perfected
    df["TD Countdown"] = td_countdown
    df["TD Countdown Direction"] = countdown_dir
    df["TD Countdown Complete"] = countdown_complete
    df["TD Signal"] = ""
    df.loc[(df["TD Setup Complete"]) & (df["TD Setup Direction"] == "Buy"), "TD Signal"] = "Buy Setup 9"
    df.loc[(df["TD Setup Complete"]) & (df["TD Setup Direction"] == "Sell"), "TD Signal"] = "Sell Setup 9"
    df.loc[(df["TD Countdown Complete"]) & (df["TD Countdown Direction"] == "Buy"), "TD Signal"] = "Buy Countdown 13"
    df.loc[(df["TD Countdown Complete"]) & (df["TD Countdown Direction"] == "Sell"), "TD Signal"] = "Sell Countdown 13"
    return df


def _extract_event_dates(event_source, price_start=None, price_end=None):
    """Return normalized event anchors as a DataFrame: date, event, category."""
    rows = []
    if event_source in ("Eclipse events", "Lunar + Eclipse"):
        e = get_eclipse_calendar_data().copy()
        if not e.empty:
            e["date"] = pd.to_datetime(e["date"], errors="coerce").dt.normalize()
            for _, r in e.iterrows():
                rows.append({"date": r["date"], "event": f"{r['type']} {r['kind']}", "category": str(r["kind"]), "source": "Eclipse", "saros": r.get("saros")})
    if event_source in ("Lunar cycles", "Lunar + Eclipse"):
        lunar_events = list(st.session_state.get("all_moon_events", []))
        for x in lunar_events:
            try:
                d = pd.Timestamp(x[0]).normalize()
                label = str(x[1]) if len(x) > 1 else "Lunar event"
            except Exception:
                continue
            rows.append({"date": d, "event": label, "category": "Lunar", "source": "Lunar", "saros": None})

        # If the app's moon-event state is unavailable, generate the phase stream
        # from ephem so the engine remains usable on a fresh session.
        if not lunar_events and price_start is not None and price_end is not None:
            cursor = pd.Timestamp(price_start).date()
            end = pd.Timestamp(price_end).date()
            while cursor <= end:
                try:
                    candidates = {
                        ephem.next_full_moon(cursor).datetime().date(): "Full Moon",
                        ephem.next_new_moon(cursor).datetime().date(): "New Moon",
                        ephem.next_first_quarter_moon(cursor).datetime().date(): "First Quarter",
                        ephem.next_last_quarter_moon(cursor).datetime().date(): "Last Quarter",
                    }
                    d = min(candidates)
                    if d > end:
                        break
                    rows.append({"date": pd.Timestamp(d), "event": candidates[d], "category": "Lunar", "source": "Lunar", "saros": None})
                    cursor = d + dt.timedelta(days=1)
                except Exception:
                    break
    if not rows:
        return pd.DataFrame(columns=["date", "event", "category", "source", "saros"])
    out = pd.DataFrame(rows).dropna(subset=["date"])
    out = out.drop_duplicates(subset=["date", "event", "category"]).sort_values("date").reset_index(drop=True)
    if price_start is not None:
        out = out[out["date"] >= pd.Timestamp(price_start).normalize()]
    if price_end is not None:
        out = out[out["date"] <= pd.Timestamp(price_end).normalize()]
    return out.reset_index(drop=True)


def build_event_cycle_dataset(price_df, event_source="Eclipse events", reset_mode="Next event", max_cycle_days=144):
    """Build one row per trading day anchored to the selected astronomical events.

    reset_mode:
      - Next event: count from the most recent event until the next event.
      - Thirds: reset at the calculated 1/3 and 2/3 boundaries as well as events.
      - Event + thirds: same as Thirds, while retaining the parent event cycle day.
    """
    prices = calculate_td_sequential(price_df)
    if prices.empty:
        return prices
    events = _extract_event_dates(event_source, prices["Date"].min(), prices["Date"].max())
    if events.empty:
        prices["Event"] = None
        prices["Event Type"] = None
        prices["Event Date"] = pd.NaT
        prices["Cycle Day"] = None
        prices["Cycle Length"] = None
        prices["Cycle %"] = None
        prices["Cycle Segment"] = None
        prices["Reset Type"] = None
        return prices

    # Map each astronomical date to the first available trading session on/after it.
    trade_dates = prices["Date"].tolist()
    event_rows = []
    for _, e in events.iterrows():
        candidates = [d for d in trade_dates if d >= e["date"]]
        if not candidates:
            continue
        anchor = candidates[0]
        row = e.to_dict()
        row["trade_date"] = anchor
        event_rows.append(row)
    anchors = pd.DataFrame(event_rows).drop_duplicates(subset=["trade_date", "event", "category"]).sort_values("trade_date")
    if anchors.empty:
        return prices

    # Each consecutive anchor defines a cycle. Counts are trading-session counts,
    # because the downstream signal is based on daily market bars.
    date_to_pos = {pd.Timestamp(d).normalize(): i for i, d in enumerate(dates)}
    cycle_records = []
    for idx, (_, anchor) in enumerate(anchors.iterrows()):
        start = pd.Timestamp(anchor["trade_date"]).normalize()
        start_pos = date_to_pos.get(start)
        if start_pos is None:
            continue
        next_start = pd.Timestamp(anchors.iloc[idx + 1]["trade_date"]).normalize() if idx + 1 < len(anchors) else None
        end_pos = date_to_pos.get(next_start) if next_start is not None else min(start_pos + int(max_cycle_days) + 1, len(dates))
        if end_pos is None or end_pos <= start_pos:
            continue
        cycle_len = max(end_pos - start_pos, 1)
        third1_pos = start_pos + max(1, int(round(cycle_len / 3)))
        third2_pos = start_pos + max(1, int(round(cycle_len * 2 / 3)))
        third1_pos = min(third1_pos, end_pos - 1)
        third2_pos = min(max(third2_pos, third1_pos + 1), end_pos - 1)
        for pos in range(start_pos, end_pos):
            d = dates[pos]
            parent_day = pos - start_pos
            segment = "Event → 1/3"
            reset_pos = start_pos
            if d >= dates[third2_pos] and reset_mode in ("Thirds", "Event + thirds"):
                segment = "2/3 → Next Event"
                reset_pos = third2_pos
            elif d >= dates[third1_pos] and reset_mode in ("Thirds", "Event + thirds"):
                segment = "1/3 → 2/3"
                reset_pos = third1_pos
            segment_day = max(0, pos - reset_pos)
            cycle_records.append({
                "Date": d, "Event": anchor["event"], "Event Type": anchor["category"],
                "Event Date": start, "Cycle Day": parent_day, "Segment Day": segment_day,
                "Cycle Length": cycle_len, "Cycle %": (parent_day / max(cycle_len - 1, 1)) * 100,
                "Cycle Segment": segment, "Reset Type": "Third" if segment != "Event → 1/3" and reset_mode in ("Thirds", "Event + thirds") else "Event",
                "Event Source": anchor["source"], "Saros": anchor.get("saros"),
            })
    cycles = pd.DataFrame(cycle_records).drop_duplicates("Date")
    result = prices.merge(cycles, on="Date", how="left")

    # Forward returns are computed from the current close using future bars. They
    # are marked as research/backtest fields and must not be used by the live signal.
    for h in [1, 3, 5, 10, 20]:
        result[f"Forward Return {h}D"] = result["Close"].shift(-h) / result["Close"] - 1
    result["Return Since Event"] = result["Close"] / result.groupby("Event Date")["Close"].transform("first") - 1
    return result


def build_event_signal_stats(cycle_df, min_samples=3):
    """Aggregate historical forward-return behavior by cycle day and TD state."""
    if cycle_df is None or cycle_df.empty:
        return pd.DataFrame()
    df = cycle_df.copy()
    df = df.dropna(subset=["Cycle Day"])
    if df.empty:
        return pd.DataFrame()
    rows = []
    group_cols = ["Event Type", "Cycle Day"]
    for keys, g in df.groupby(group_cols, dropna=False):
        row = {"Event Type": keys[0], "Cycle Day": int(keys[1]), "Samples": len(g)}
        for h in [1, 3, 5, 10, 20]:
            col = f"Forward Return {h}D"
            vals = pd.to_numeric(g[col], errors="coerce").dropna()
            row[f"Avg {h}D %"] = float(vals.mean() * 100) if not vals.empty else None
            row[f"Positive {h}D %"] = float((vals > 0).mean() * 100) if not vals.empty else None
        td = g["TD Signal"].replace("", pd.NA).dropna()
        row["TD Signals"] = ", ".join(sorted(td.unique())[:5]) if not td.empty else ""
        rows.append(row)
    out = pd.DataFrame(rows)
    if out.empty:
        return out
    out = out[out["Samples"] >= int(min_samples)].sort_values(["Event Type", "Cycle Day"]).reset_index(drop=True)
    return out


def get_current_event_signal(cycle_df, ticker=None):
    """Return the latest known event-cycle state for the active ticker."""
    if cycle_df is None or cycle_df.empty:
        return None
    row = cycle_df.sort_values("Date").iloc[-1].to_dict()
    return row


def render_event_signal_monitor(ticker, price_df):
    """Interactive Bokeh monitor for event-cycle state and historical qualification."""
    st.markdown("### Event Signal Monitor")
    st.caption("Event dates anchor a daily trading-session count. TD Sequential is a confirmation layer; forward-return fields are historical/backtest data and are never used to create the live row.")
    if price_df is None or price_df.empty:
        st.info("Load price history before opening the Event Signal Monitor.")
        return
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        event_source = st.selectbox("Event source", ["Lunar cycles", "Eclipse events", "Lunar + Eclipse"], key="event_signal_source")
    with c2:
        reset_mode = st.selectbox("Cycle reset", ["Next event", "Thirds", "Event + thirds"], key="event_signal_reset")
    with c3:
        max_cycle = st.selectbox("Max cycle", [29, 36, 45, 72, 90, 144, 180], index=5, key="event_signal_max_cycle")
    with c4:
        min_samples = st.number_input("Min historical samples", 1, 100, 3, 1, key="event_signal_min_samples")

    cycle_df = build_event_cycle_dataset(price_df, event_source, reset_mode, int(max_cycle))
    if cycle_df.empty:
        st.info("No event anchors overlap the loaded price history.")
        return
    current = get_current_event_signal(cycle_df, ticker)
    stats = build_event_signal_stats(cycle_df, int(min_samples))

    if current:
        m1, m2, m3, m4, m5 = st.columns(5)
        m1.metric("Cycle Day", f"{int(current['Cycle Day']) if pd.notna(current['Cycle Day']) else '—'}")
        m2.metric("Cycle %", f"{float(current['Cycle %']):.1f}%" if pd.notna(current['Cycle %']) else "—")
        m3.metric("TD Setup", f"{int(current['TD Setup'])}/9" if current.get("TD Setup") else "—")
        m4.metric("TD Countdown", f"{int(current['TD Countdown'])}/13" if current.get("TD Countdown") else "—")
        m5.metric("TD State", current.get("TD Signal") or current.get("TD Setup Direction") or "Neutral")

    # Historical qualification for the current cycle position. Only show completed
    # forward outcomes, never future values from the current row as a live signal.
    current_day = int(current["Cycle Day"]) if pd.notna(current.get("Cycle Day")) else None
    current_type = current.get("Event Type")
    if current_day is not None and not stats.empty:
        match = stats[(stats["Event Type"] == current_type) & (stats["Cycle Day"] == current_day)]
        if not match.empty:
            r = match.iloc[0]
            cols = st.columns(5)
            for col, h in zip(cols, [1, 3, 5, 10, 20]):
                avg = r.get(f"Avg {h}D %")
                hit = r.get(f"Positive {h}D %")
                col.metric(f"Hist {h}D", f"{avg:+.2f}%" if pd.notna(avg) else "—", f"{hit:.0f}% positive" if pd.notna(hit) else None)
            st.caption(f"Historical samples at this exact event type/cycle day: {int(r['Samples'])}.")

    # Bokeh price + cycle markers.
    if BOKEH_AVAILABLE:
        plot = cycle_df.copy()
        plot["Cycle Day"] = pd.to_numeric(plot["Cycle Day"], errors="coerce")
        source = ColumnDataSource(plot)
        fig = figure(x_axis_type="datetime", height=500, sizing_mode="stretch_width", title=f"{str(ticker).upper()} — Event Cycle / TD Sequential")
        fig.line("Date", "Close", source=source, line_width=2, alpha=0.75, legend_label="Close")
        fig.circle("Date", "Close", source=source, size=5, alpha=0.0, hover_alpha=0.9)
        fig.add_tools(HoverTool(tooltips=[("Date", "@Date{%F}"), ("Event", "@Event"), ("Cycle Day", "@{Cycle Day}"), ("Cycle %", "@{Cycle %}{0.0}"), ("TD Setup", "@{TD Setup}"), ("TD Countdown", "@{TD Countdown}"), ("TD Signal", "@{TD Signal}"), ("Close", "@Close{0.00}")], formatters={"@Date": "datetime"}, mode="vline"))
        fig.legend.click_policy = "hide"
        fig.xaxis.axis_label = "Date"
        fig.yaxis.axis_label = "Price"
        components.html(file_html(fig, CDN, title="Event Signal Monitor"), height=540, scrolling=False)

    display_cols = ["Date", "Event", "Event Type", "Event Date", "Cycle Day", "Segment Day", "Cycle %", "Cycle Segment", "TD Setup", "TD Setup Direction", "TD Setup Perfected", "TD Countdown", "TD Countdown Direction", "TD Signal", "Close", "Forward Return 1D", "Forward Return 5D", "Forward Return 10D", "Forward Return 20D"]
    available = [c for c in display_cols if c in cycle_df.columns]
    grid = cycle_df[available].tail(250).copy()
    for c in ["Date", "Event Date"]:
        if c in grid.columns:
            grid[c] = pd.to_datetime(grid[c], errors="coerce").dt.strftime("%Y-%m-%d")
    for c in [x for x in grid.columns if "Return" in x]:
        grid[c] = pd.to_numeric(grid[c], errors="coerce") * 100
    st.markdown("#### Daily cycle tape")
    render_aggrid(grid, height=430, key="event_signal_daily_grid")

    if not stats.empty:
        st.markdown("#### Historical event qualification")
        stats_display = stats.copy()
        for c in [x for x in stats_display.columns if c.endswith("%") and ("Avg" in c or "Positive" in c)]:
            stats_display[c] = pd.to_numeric(stats_display[c], errors="coerce").round(2)
        render_aggrid(stats_display.head(500), height=360, key="event_signal_stats_grid")
...
def main_app():
    ...
    def display_algo_trade_replay():
        ...
        # Render the complete Bokeh document through Streamlit's HTML component.
        #
        # Streamlit Cloud deployments can run a Streamlit build where
        # ``st.bokeh_chart`` is unavailable or where its Bokeh serialization
        # path is incompatible with the installed Bokeh version.  Calling it
        # directly caused the AttributeError reported from this line.  The
        # Bokeh figures/widgets themselves are valid, so embed the same layout
        # as a standalone Bokeh document instead. This keeps the existing
        # client-side slider/CustomJS replay behavior intact.
        algo_replay_layout = column(slider, price_fig, option_fig, pnl_fig, sizing_mode="stretch_width")
        algo_replay_html = file_html(
            algo_replay_layout,
            CDN,
            title="Alpha Algo Trade Simulator Replay",
        )
        components.html(
            algo_replay_html,
            height=1040,
    ...
    with tab5:
        ...
        st.markdown("---")
        render_event_signal_monitor(st.session_state.get("ticker", current_ticker), st.session_state.get("stock_data"))
        st.markdown("---")
